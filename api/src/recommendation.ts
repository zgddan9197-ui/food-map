...
const scoreByScene = (place: Place, scene: SceneInput): number => {
  let score = 0
...
const scoreByIntent = (place: Place, freeText: string, aiHints: string[]): number => {
  const tokens = extractIntentTokens(`${freeText} ${aiHints.join(' ')}`)
  if (tokens.length === 0) {
    return 0
  }
...
const scoreByTrust = (place: Place): number => {
...
const scoreByDistance = (
  distanceKm: number,
  scene: SceneInput,
  serviceSpeed: Place['mealServiceSpeed'],
): number => {
...
const scoreByInterestBoost = (place: Place, boost: Record<string, number>): number => {
...
const scorePlace = (
  ...
  const distanceKm = calcDistanceKm(campus.center, place.location)
  const walkMinutes = estimateWalkMinutes(distanceKm)
  const openNow = isOpenNow(place.openHours)
...
export const selectNextBatch = (
  ranked: PlaceWithRecommendation[],
  ...
  const picked: PlaceWithRecommendation[] = []
  const pickedIds = new Set<string>()

  const pickFrom = (pool: PlaceWithRecommendation[], limit: number): void => {
