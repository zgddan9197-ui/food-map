...
import { campusMap, campuses, placeMap, places } from './state'
import { createPgMirror } from './postgres'
import { RateLimiter } from './rate-limit'
import { buildRankedRecommendations, selectNextBatch } from './recommendation'
import { MemoryStore } from './store'
import { buildTrustSnapshot } from './trust'
import type { PlaceWithRecommendation, SceneInput } from './types'
...
const mirror = createPgMirror(process.env.DATABASE_URL)
const limiter = new RateLimiter(process.env.REDIS_URL, Number(process.env.RATE_LIMIT_PER_MINUTE ?? '120'))
const adminKey = process.env.ADMIN_KEY ?? 'shike-admin'

const extractSessionToken = (authorization: string | undefined): string | null => {
  if (!authorization) {
    return null
  }
...
const ensureSession = (authorization: string | undefined) => {
...
const parseOrReply = <T extends z.ZodTypeAny>(
  schema: T,
  payload: unknown,
): { ok: true; data: z.infer<T> } | { ok: false; error: unknown } => {
...
const toRecommendationResponse = (item: PlaceWithRecommendation) => ({
  place: {
    id: item.place.id,
...
const qualityWeight = (text: string): number => {
...
const buildFilteredRanked = async (
  scene: SceneInput,
  interestBoost: Record<string, number>,
  options: { openNowOnly: boolean; maxWalkMinutes: number },
): Promise<PlaceWithRecommendation[]> => {
  const campus = campusMap[scene.campusId]
  if (!campus) {
    return []
  }

  const scopedPlaces = places.filter((item) => item.campusId === campus.id)
  const aiHints = await ai.extractIntentHints(scene.freeText)
  const ranked = buildRankedRecommendations(scopedPlaces, scene, campus, interestBoost, aiHints)

  return ranked.filter(
    (item) => (!options.openNowOnly || item.openNow) && item.recommendation.walkMinutes <= options.maxWalkMinutes,
  )
}

const routePoints = (batch: PlaceWithRecommendation[]) =>
...
app.post('/api/v1/ai/supplement-places', async (request, reply) => {
  const parsed = parseOrReply(
    z.object({
      ...
      maxItems: z.number().int().min(1).max(4).optional().default(2),
    }),
    request.body,
  )
...
app.post('/api/v1/anon/session', async (request, reply) => {
  ...
  const deviceId = parsed.data.deviceId ?? `dev_${randomUUID()}`
...
app.post('/api/v1/recommendations', async (request, reply) => {
  ...
  const parsed = parseOrReply(
    z.object({
      campusId: z.string(),
      people: z.enum(['solo', 'smallGroup', 'group']),
      mealTime: z.enum(['breakfast', 'lunch', 'dinner', 'lateNight']),
      urgency: z.enum(['rush', 'normal', 'relax']),
      freeText: z.string().max(200).default(''),
      openNowOnly: z.boolean().optional().default(false),
      maxWalkMinutes: z.number().int().min(8).max(60).optional().default(30),
    }),
    request.body,
  )
  ...
  const campus = campusMap[parsed.data.campusId]
  if (!campus) {
  ...
  const firstBatch = selectNextBatch(filteredRanked, new Set(), new Set(), 5)
  ...
  void mirror.saveRecommendations(
    firstBatch.map((item) => ({
...
app.post('/api/v1/recommendations/:sceneId/refresh', async (request, reply) => {
  ...
  const paramsParsed = parseOrReply(z.object({ sceneId: z.string() }), request.params)
  if (!paramsParsed.ok) {
    return reply.code(400).send({ error: 'invalid_scene_id', details: paramsParsed.error })
  }

  const bodyParsed = parseOrReply(
    z.object({
      previousIds: z.array(z.string()).max(20).default([]),
      openNowOnly: z.boolean().optional().default(false),
      maxWalkMinutes: z.number().int().min(8).max(60).optional().default(30),
      interestBoost: z.record(z.string(), z.number()).optional().default({}),
    }),
    request.body,
  )
  if (!bodyParsed.ok) {
  ...
  const filteredRanked = await buildFilteredRanked(
    sceneSession.scene,
    bodyParsed.data.interestBoost,
    bodyParsed.data,
  )

  const previousIds = new Set(bodyParsed.data.previousIds)
  const seenIds = store.getSceneSeenIds(sceneSession.id)
  const nextBatch = selectNextBatch(filteredRanked, previousIds, seenIds, 5)
...
app.get('/api/v1/restaurants/:id', async (request, reply) => {
  const parsed = parseOrReply(z.object({ id: z.string() }), request.params)
  if (!parsed.ok) {
    return reply.code(400).send({ error: 'invalid_id', details: parsed.error })
  }

  const place = placeMap[parsed.data.id]
  if (!place) {
  ...
  const snapshot =
    cached ??
    (await buildTrustSnapshot(
      place,
      store.getReviewsByPlace(place.id),
      store.getRecentDecisionsForPlace(place.id, 30),
      ai,
    ))
...
app.post('/api/v1/decisions/go', async (request, reply) => {
  ...
  const place = placeMap[parsed.data.placeId]
  if (!place) {
    return reply.code(404).send({ error: 'place_not_found' })
  }
...
app.get('/api/v1/community/posts', async (request, reply) => {
  ...
  const rows = store
    ...
    .map((post) => ({
      ...post,
      placeName: post.placeId ? placeMap[post.placeId]?.name ?? null : null,
...
app.post('/api/v1/community/posts', async (request, reply) => {
  ...
  if (parsed.data.placeId && !placeMap[parsed.data.placeId]) {
    return reply.code(404).send({ error: 'place_not_found' })
  }
...
app.post('/api/v1/community/posts/:postId/comments', async (request, reply) => {
  ...
  const paramsParsed = parseOrReply(z.object({ postId: z.string() }), request.params)
  if (!paramsParsed.ok) {
...
app.post('/api/v1/submissions/place', async (request, reply) => {
  ...
  const parsed = parseOrReply(
    z.object({
      ...
      type: z.enum(['restaurant', 'stall']),
      campusId: z.string(),
      addressHint: z.string().min(2).max(120),
  ...
  if (!campusMap[parsed.data.campusId]) {
    return reply.code(404).send({ error: 'campus_not_found' })
  }
...
app.post('/api/v1/admin/moderation/:id/action', async (request, reply) => {
  ...
  const paramsParsed = parseOrReply(z.object({ id: z.string() }), request.params)
  if (!paramsParsed.ok) {
